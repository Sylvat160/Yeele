...
<head>
    ...
    <title>Yeele | Modification de données</title>
    ...
    <div class="w-100 min-h-screen py-4 bg-gray-100/2">
        <div class="bg-slate-100 w-11/12 rounded-lg border border-gray-200 shadow-sm mx-auto"
            ...
            <form class="px-4 py-8" action="{{ route('participant.update') }}" method="POST"
                data-participant-additional-data="{{ $participant->additional_data }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <select name="civility" id="civility"
                        ...
                        @foreach (['Monsieur', 'Madame', 'Mademoiselle'] as $civility)
                            <option value="{{ $civility }}" @if ($civility === $participant->civility)  @endif>
                                {{ $civility }}</option>
                        @endforeach
                    </select>
                    @error('civility')
                ...
                @if ($event->form)
                    @foreach ($fields as $field)
                        @switch($field['type'])
                            @case('header')
                                <div class="mb-3">
                                    <{{ $field['subtype'] }}>{{ str_replace('<br>', '', $field['label']) }}
                                        </{{ $field['subtype'] }}>
                                </div>
                            @break
                                    ...
                                    <select name="{{ $field['name'] }}" id="{{ $field['name'] }}"
                                        ...
                                        @foreach ($field['values'] as $option)
                                            <option value="{{ $option['value'] }}"
                                                @if ($option['selected']) selected @endif>{{ $option['label'] }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
...
@endisset
                                ...
                                <div class="mb-3">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            id="{{ $field['name'] }}" name="{{ $field['name'] }}" type="file"
                                            @if ($field['required']) required @endif>
                                    </div>
                                </div>
                            @break
                    ...
                    <div class="mb-3">
                        ...
                        @if ($event->multiple_prices_active && !$event->custom['hasDirectPayment'])
                            @if ($event->prices_quantity_active)
                                <input type="hidden" name="prices" value="{{ $participant->custom['pwithq'] }}"
                                    required>
                            @else
                                <input type="hidden" name="prices" value="{{ $participant->custom['pwithoutq'] }}"
                                    required>
                            @endif
                                            ...
                                            <div class="w-full flex items-center p-2 rounded hover:bg-gray-100">
                                                @if ($event->prices_quantity_active)
                                                    <input id="{{ $price->uid }}" type="checkbox"
                                                        ...
                                                        @if (in_array([$price->uid => $price->quantity], $participant->custom['pwithq']))  @endif>
                                                @else
                                                    <input id="{{ $price->uid }}" type="checkbox"
                                                        value="{{ $price->uid }}" data-value="{{ $price->value }}"
                                                        class="w-4 h-4 checked:bg-red-500 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-red-300 price"
                                                        @if (in_array($price->uid, json_decode($participant->custom['pwithoutq'], true))) checked @endif>
                                                @endif
                                                <label for="{{ $price->uid }}"
                                ...
                                @if ($event->eventPrices->count() > 1)
                                    @foreach ($event->eventPrices as $price)
                                        <option value="{{ $price->value }}"
                                            @if ($price->value === $participant->price) selected @endif>{{ $price->label }}
                                            ({{ $price->value . ' FCFA' }}
                                            FCFA)</option>
                                    @endforeach
                                @else
                                ...
                                @if ($event->event_payment_methods->count() > 1)
                                    ...
                                    @foreach ($event->event_payment_methods as $method)
                                        <option value="{{ $method->name }}"
                                            @if ($method->name === $participant->payment_method) selected @endif>{{ $method->name }}
                                        </option>
                                    @endforeach
                ...
                <div class="mt-4 text-center">
                    <a href="{{ route('participants-edit-list', $event->uid) }}"
                        class="flex justify-center items-center text-red-500 focus:text-red-600 font-medium text-sm">
